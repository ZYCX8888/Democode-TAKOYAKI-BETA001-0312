...
static ssize_t measure_all_hw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < iMiuClientNum; i++)
    {
        if ((pstMiuClient + i)->bw_enabled && (pstMiuClient + i)->bw_rsvd == 0)
        {
            ...
            (pstMiuClient + i)->effi_avg = INREG16((iMiuBankAddr+REG_ID_0E));
            OUTREG16((iMiuBankAddr+REG_ID_0D), 0) ; // reset all


            OUTREG16((iMiuBankAddr+REG_ID_0D), ((id << 8) | 0x40)); // reset
            OUTREG16((iMiuBankAddr+REG_ID_29), 0x0303);
            OUTREG16((iMiuBankAddr+REG_ID_0D), ((id << 8) | 0x41)); // set to read average bandwidth
            msleep(iMonitorDuration);
            (pstMiuClient + i)->bw_avg=INREG16((BASE_REG_MIU_PA+REG_ID_0E));
            OUTREG16((iMiuBankAddr+REG_ID_0D), 0) ; // reset all


            OUTREG16((iMiuBankAddr+REG_ID_0D), ((id << 8) | 0x50)); // reset
            OUTREG16((iMiuBankAddr+REG_ID_29), 0x0303);
            OUTREG16((iMiuBankAddr+REG_ID_0D), ((id << 8) | 0x51)); // set to read peak bandwidth
            msleep(iMonitorDuration);
            (pstMiuClient + i)->bw_max=INREG16((BASE_REG_MIU_PA+REG_ID_0E));
            OUTREG16((iMiuBankAddr+REG_ID_0D), 0) ; // reset all


            OUTREG16((iMiuBankAddr+REG_ID_0D), ((id << 8) | 0x20)) ;//reset
            OUTREG16((iMiuBankAddr+REG_ID_29), 0x0303);
            OUTREG16((iMiuBankAddr+REG_ID_0D), ((id << 8) | 0x21)) ;//set to read average
            msleep(iMonitorDuration);
