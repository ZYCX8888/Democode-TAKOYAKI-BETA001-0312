...
static int _MHal_EMAC_DPHY_REINIT(void* hal)
{
    ...
    for(i=0; i<200; i++)
    {
        ...
        if(uRegVal_7E == 0x08 && uRegVal_A5 == 0x07)
        {
            ...
            if(uRegVal_8x > -64 && uRegVal_8x < 64)
            {
                cntGood++;
                cntBad = 0;
                if(cntGood > 5)
...
static void _MHal_EMAC_albany_clk_on(void* hal)
{
    ...
    if(Chip_Get_Revision() == 0x1)
    {
        ...
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x76, 0x00); //1515 3b
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x77, 0x18);

        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x90, 0x04); //1515 48
