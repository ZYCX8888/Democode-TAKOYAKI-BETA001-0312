		...
		if (nand_block_isbad(nand, offset & ~(nand->erasesize - 1)))
             {
				...
				offset & ~(nand->erasesize - 1));
                    ...
                    if(left_to_write > lim)
                    {
                        printf("Number of bad block exceeds partition limit!\n");
                        printf("Partition mark bad\n");
                        left_to_write= 0;
                        ...
                        while(partSize > 0)
                        {
                            ...
                            if (mtd_block_markbad(nand, partOffset)) {
                            ...
                            partOffset += nand->erasesize - block_offset;
                            partSize -= blocksize;
                        }
                    }
                    else
                    {
                        /* If WB; handle BBM , swap block */
                        SPI_NAND_DRIVER_t *pNandDrv = (SPI_NAND_DRIVER_t*)drvSPINAND_get_DrvContext_address();

                        if (pNandDrv->tSpinandInfo.au8_ID[0]==0xEF && pNandDrv->tSpinandInfo.au8_ID[1]==0xAA)
                        {
                              ...
                              {
                                    if (!get_part("UBI", &idx, &off, &size, &maxsize))
                                    {
                                        if (block_offset >= off)
                                           printf("[sstar] Skip UBI \r\n");
                                        else
                                        {
                                            /* Swap */
                                            printf("[sstar] start winbond bbm \r\n");
                                            ...
                                            if(!bbmCounter)
                                            {
                                                offset -= nand->erasesize - block_offset;
                                                /* get bad block index*/
                                                LBA = offset >> MDrv_SPINAND_CountBits(nand->erasesize);
                                                /* get good block from last block of parition */
                                                PBA = (badOffset +  partLimit - nand->erasesize);

                                                if(!nand_block_isbad(nand, PBA))
                                                {   
                                                    PBA = PBA >> MDrv_SPINAND_CountBits(nand->erasesize);//swap partition' end block
                                                ...
                                                {
                                                    printf("[sstar] last block of parition is bad block !!!\r\n");
                                                }
                                            }
                                            else
                                            {
                                                printf("[sstar] BBM over 2 bad block  of partition !!!\r\n");
                                            }
                                        }
                                    }
                              }
