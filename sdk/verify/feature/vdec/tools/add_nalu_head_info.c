...
#define MAX_NAIL_BUFF_SIZE (1024*1024*10)
...
static S32 WriteHeadInfo(FILE *pEsFile, U32 u32FrmLen)
{
...
static S32 WriteOneNalu(NALU_t *pstNalu, FILE   *fpStream)
{
    CHECK_NULL_POINTER(__FUNCTION__, pstNalu);
    CHECK_NULL_POINTER(__FUNCTION__, fpStream);

    //每一个Nalu前面添加16字节的头信息
    if(SUCCESS != WriteHeadInfo(fpStream, pstNalu->len))
    {
        printf("Func:%s, Line:%d, WriteHeadInfo failed!\n", __FUNCTION__, __LINE__);
        return FAILURE;
    }
...
S32 GetAnnexbNALU(NALU_t *pstNalu, FILE     *fpStream)
{
    ...
    while(!bStartCodeFound)
    {
        ...
        if(s32Pos >= MAX_NAIL_BUFF_SIZE)
            printf("%s,%d, s32Pos[%d] nal over buffer size[%d]\n", __FUNCTION__, __LINE__, s32Pos, MAX_NAIL_BUFF_SIZE);
...
S32 H26x_parser(S8 *ps8SrcFileName, S8 *ps8DstFileName)
{
    ...
    while(!feof(fpSrcFile))
    {
        ...
        if(SUCCESS !=  WriteOneNalu(&stNalu, fpDstFile))
        {
            printf("Func:%s, Line:%d, WriteOneNalu failed!\n", __FUNCTION__, __LINE__);
            return FAILURE;
        }
    }
