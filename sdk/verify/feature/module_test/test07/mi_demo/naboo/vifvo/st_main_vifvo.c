...
#if USE_FD
    s32FdStream = MI_VENC_GetFd(pArgs->stVencAttr[0].vencChn);
    if(s32FdStream <= 0)
    {
        printf("Unable to get FD:%d for ch:%2d\n", s32FdStream, pArgs->stVencAttr[0].vencChn);
        return NULL;
    }
    else
    {
        printf("CH%2d FD%d\n", pArgs->stVencAttr[0].vencChn, s32FdStream);
    }
#endif
    while (1)
    {
        ...
        {
#if POLL
#error NOT ported. Refer venc_common.c
#else
            MI_VENC_ChnStat_t stStat;
            fd_set fdsread;
            struct timeval tv;
    ...
    if(s32FdStream > 0)
    {
        s32Ret = MI_VENC_CloseFd(pArgs->stVencAttr[0].vencChn);
        if(s32Ret != 0)
        {
            printf("CH%02d Ret:%X\n", pArgs->stVencAttr[0].vencChn, s32Ret);
        }
        s32FdStream = -1;
    }
#endif
...
MI_S32 ST_CreateVencChannel(MI_S32 s32VencChn, MI_S32 s32VencType, MI_U16 u16Width, MI_U16 u16Height, MI_S32 s32FrameRate)
{
    ...
    s32Ret = MI_VENC_SetMaxStreamCnt(s32VencChn, 8);
    if (MI_SUCCESS != s32Ret)
    {
...
MI_S32 ST_VifToDisp(MI_S32 s32CaseIndex)
{
    ...
    *************************************************/
    ...
    for (i = 0; i < s32DispChnNum; i++)
    {
        ...
        //ExecFunc(MI_SYS_SetChnOutputPortDepth(&stVpeChnOutputPort, 0, 6), 0);
