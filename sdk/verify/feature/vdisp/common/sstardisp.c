...
stTimingArray_t *gettiming(MI_DISP_OutputTiming_e eOutputTiming)
{
    int index = 0;
...
int sstar_disp_init(MI_DISP_PubAttr_t *pstDispPubAttr)
{
    ...
    {
        //stVideoLayerAttr.ePixFormat = ;
        //stVideoLayerAttr.stVidLayerDispWin = ;
        //stVideoLayerAttr.stVidLayerSize = ;
    ...
    else if (pstDispPubAttr->eIntfType == E_MI_DISP_INTF_LCD)
    {
        ...
        pstDispPubAttr->u32BgColor = YUYV_GREEN;
    ...
    if(pstDispPubAttr->eIntfType == E_MI_DISP_INTF_LCD) {
        ...
        /*
        MI_U32 u32SpiCmdCnt, n;
        MI_PANEL_GpioConfig_t stGpioCfg;
        memset(&stGpioCfg, 0, sizeof(MI_PANEL_GpioConfig_t));
        stGpioCfg.u16GpioBL = UNNEEDED_CONTROL;
        stGpioCfg.u16GpioRST = 11;
        stGpioCfg.u16GpioCS = 8;
        stGpioCfg.u16GpioSCL = 9;
        stGpioCfg.u16GpioSDO = 10;
        stGpioCfg.u16GpioEN = UNNEEDED_CONTROL;
        MI_PANEL_GPIO_Init(&stGpioCfg);
        //MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioBL, 1);
        MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioCS, 1);
        MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioSCL, 1);
        MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioSDO, 1);
        MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioRST, 0);
        usleep(20 * 1000);
        MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioRST, 1);
        ...
        for(n = 0; n < u32SpiCmdCnt; n++) {
            if(REGFLAG_DELAY == Initialization_CmdTable[n].cmd)
                ...
                MI_PANEL_SetGpioStatus(stGpioCfg.u16GpioCS, 0);
