...
typedef struct
{
    int         charNumPerLine; // ??DD/°ß°ÈD??°ßo?|®¨??®¢??°Ë?°ßoy, ?D??°ß°„2°ß°„????®¢??°Ë?
    int         bgColor;        // ?®§3???°ß|?
    int         fgColor;        // ??????°ß|?
    int         leftMargin;     // margin °ßo?D?
    int         rightMargin;
    int         upMargin;
    int         downMargin;
    int         verticalFlag;   // °ßo°ß2?????
    int         charSpace;      // ?®¢??°Ë????°ß°Ë
    int         lineSpace;      // DD???°ß°Ë
} DMF_BitMapAttr_S;
...
int _OSD_CalcCharNumGetGb2312Code(const char *string, int *charTotalNum, uint8_t *gb2312buf, int bufLen)
{
    ...
    else if (enEncodIngType == GBK)
    {
        ...
        do
        {
            enEncodIngType = _OSD_DetectEncodingType(ptr);
...
{
    DMF_BitMapFile_S *pstDMFBitMapFile = &g_dmf_bitmapfile[0][0];
...
void _OSD_FontDataToCanvas(uint8_t *pFontdata, int x, int y, int width,
...
#if 0
    for (i = 0; i < height; i ++) // |®¨1°ß°È?®¶|®¨???°ßa?
#else
    for (i = height - 1; i >= 0; i--) // ?y°ß°È?®¶|®¨???°ßa?
    {
...
MI_S32 ST_OSD_Init(MI_RGN_HANDLE hHandle, ST_OSD_Attr_T *pstOsdAttr)
{
    ...
    stRgnAttr.stOsdInitParam.ePixelFmt = pstOsdAttr->ePixelFmt;
    ...
    stRgnChnPortParam.stPoint.u32X = pstOsdAttr->stRect.u32X;
    stRgnChnPortParam.stPoint.u32Y = pstOsdAttr->stRect.u32Y;
    stRgnChnPortParam.unPara.stOsdChnPort.u32Layer = pstOsdAttr->u32Layer;
    // stRgnChnPortParam.unPara.stCoverChnPort.u32Layer = g_stRgnInfo[hHandle].hHandle;
    // stRgnChnPortParam.unPara.stCoverChnPort.stSize.u32Width = RGN_OSD_TIME_WIDTH;
    // stRgnChnPortParam.unPara.stCoverChnPort.stSize.u32Height = RGN_OSD_TIME_HEIGHT;
    // stRgnChnPortParam.unPara.stCoverChnPort.u32Color = 0;
...
MI_S32 ST_OSD_GetCanvasInfo(MI_RGN_HANDLE hHandle, MI_RGN_CanvasInfo_t** ppstCanvasInfo)
{
    ...
    s32Ret = MI_RGN_GetCanvasInfo(hHandle, &g_stRgnInfo[hHandle].stCanvasInfo);
