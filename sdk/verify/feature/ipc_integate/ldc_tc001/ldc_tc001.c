...
static MI_S32 St_WriteFile(Test_Parames_t     *pstParamer)
{
    ...
    stSDChnOutputPort.eModId = E_MI_MODULE_ID_LDC;
...
static MI_S32 St_BaseModuleInit(Test_Parames_t     *pstParamer)
{
    ...
    *************************************************/
    ...
    MI_LDC_CHN LdcChannel = 0;
    MI_LDC_ChannelAttr_t  stLdcChnlAttr;
    MI_LDC_OutputPortAttr_t stLdcPortMode;
    memset(&stLdcPortMode, 0x0, sizeof(stLdcPortMode));
    memset(&stLdcChnlAttr, 0x0, sizeof(stLdcChnlAttr));

    MI_LDC_CreateDevice(ldc_devid, E_LDC_OFF_MODE);

    stLdcChnlAttr.u16MaxW = 1920;
    stLdcChnlAttr.u16MaxH = 1080;
    MI_LDC_CreateChannel(ldc_devid,LdcChannel, &stLdcChnlAttr);

    stLdcPortMode.ePixelFmt = pstParamer->eOutPixelFormat;
    stLdcPortMode.u16Width = pstParamer->u16OutWidth;
    stLdcPortMode.u16Height = pstParamer->u16OutHeight;
    MI_LDC_SetOutputPortAttr(ldc_devid, LdcChannel, &stLdcPortMode);

    MI_LDC_StartChannel(ldc_devid, LdcChannel);

    stChnPort.eModId = E_MI_MODULE_ID_LDC;
    stChnPort.u32DevId = 0;
    stChnPort.u32ChnId = LdcChannel;
    ...
    *************************************************/
    ...
    stBindInfo.stDstChnPort.u32DevId = 0;
    stBindInfo.stDstChnPort.u32ChnId = LdcChannel;
    stBindInfo.stDstChnPort.u32PortId = 0;
    stBindInfo.u32SrcFrmrate = 30;
    stBindInfo.u32DstFrmrate = 30;
    STCHECKRESULT(ST_Sys_Bind(&stBindInfo));
...
static MI_S32 St_BaseModuleDeinit(Test_Parames_t     * pstParamer)
{
    ...
    STCHECKRESULT(MI_LDC_StopChannel(0,0));
    STCHECKRESULT(MI_LDC_DestroyChannel(0,0));
    STCHECKRESULT(MI_LDC_DestroyDevice(0));
