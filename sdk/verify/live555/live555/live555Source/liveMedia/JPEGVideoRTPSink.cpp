...
::JPEGVideoRTPSink(UsageEnvironment& env, Groupsock* RTPgs)
  : VideoRTPSink(env, RTPgs, 26, 90000, "JPEG"), fFmtpSDPLine(NULL) {
}
...
char const* JPEGVideoRTPSink::auxSDPLine() {
  ...
  JPEGVideoSource* source = (JPEGVideoSource*)fSource;
  u_int16_t nWidth=0, nHeight=0;

  if(!fSource)
    return NULL;

  nWidth = source->widthPixels();
  nHeight = source->heightPixels();

  if((0==nWidth)||(0==nHeight))
    return NULL;

  char pResolutionString[16] = {0};  // max value is 4000x3000
  sprintf(pResolutionString, "%d,%d", nWidth, nHeight);

  char const* fmtpFmt = "a=x-dimensions:%s\r\n";
  unsigned fmtpFmtSize = strlen(fmtpFmt)
                         + strlen(pResolutionString)
                         + 1 /* For '\0' */;
  char* fmtp = new char[fmtpFmtSize];
  sprintf(fmtp, fmtpFmt, pResolutionString);
