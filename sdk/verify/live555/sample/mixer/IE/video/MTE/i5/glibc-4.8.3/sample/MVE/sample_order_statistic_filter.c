/*
* sample_order_statistic_filter.c- Sigmastar
*
...
#define OUTPUT_NAME  "Output_OrdStatFilter.raw"

MI_RET Sample_OrdStatFilter()
{
    MI_RET ret;
    MVE_HANDLE handle;
	...
	MVE_SRC_IMAGE_S src;
	MVE_DST_IMAGE_S dst;
    MVE_ORD_STAT_FILTER_CTRL_S ctrl =
    {
        .enMode = MVE_ORD_STAT_FILTER_MODE_MEDIAN
    };
    ...
    // Run MI_MVE_OrdStatFilter()
	ret = MI_MVE_OrdStatFilter(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_OrdStatFilter() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
