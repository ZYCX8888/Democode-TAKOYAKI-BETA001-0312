...
#define OUTPUT_NAME     "Output_Sub.raw"
...
MI_RET Sample_Sub()
{
    ...
    // Run MI_MVE_Sub
    ret = MI_MVE_Sub(handle, &src0, &src1, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_Sub() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
