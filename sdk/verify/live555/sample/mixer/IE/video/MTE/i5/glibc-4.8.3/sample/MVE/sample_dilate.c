...
#define OUTPUT_NAME  "Output_Dilate.raw"

MI_RET Sample_Dilate()
{
    MI_RET ret;
    MVE_HANDLE handle;
    ...
    // Run MI_MVE_Dilate()
	ret = MI_MVE_Dilate(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_Dilate() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
