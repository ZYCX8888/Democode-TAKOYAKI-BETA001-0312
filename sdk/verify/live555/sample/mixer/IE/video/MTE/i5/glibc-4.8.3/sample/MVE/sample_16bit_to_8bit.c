/*
* sample_16bit_to_8bit.c- Sigmastar
*
...
MI_RET Sample_16BitTo8Bit()
{
	...
	MVE_SRC_IMAGE_S src;
	MVE_DST_IMAGE_S dst;
    MVE_16BIT_TO_8BIT_CTRL_S ctrl =
    {
        .enMode         = MVE_16BIT_TO_8BIT_MODE_U16_TO_U8,
        .u16Denominator = 256,
        .u8Numerator    = 1,
        .s8Bias         = 0
    };
    ...
    // Run MI_MVE_16BitTo8Bit()
	ret = MI_MVE_16BitTo8Bit(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_16BitTo8Bit() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
