...
MI_RET Sample_FilterAndCSC()
{
	...
	MVE_SRC_IMAGE_S src;
	MVE_DST_IMAGE_S dst;
    MVE_FILTER_AND_CSC_CTRL_S ctrl =
    {
        .enMode = MVE_CSC_MODE_PIC_BT601_YUV2RGB,
        .as8Mask =
        {
        //Gaussian filter
             0, 0, 0, 0, 0,
    ...
    ret = MI_MVE_AllocateImage(handle, &src, MVE_IMAGE_TYPE_YUV420SP, RAW_WIDTH, RAW_WIDTH, RAW_HEIGHT, "MVE_Input", MVE_MEM_ALLOC_TYPE_PHYSICAL);
    ...
    ret = MI_MVE_AllocateImage(handle, &dst, MVE_IMAGE_TYPE_U8C3_PACKAGE, RAW_WIDTH, RAW_WIDTH, RAW_HEIGHT, "MVE_Output", MVE_MEM_ALLOC_TYPE_PHYSICAL);
    if (ret != MI_RET_SUCCESS)
    {
    ...
    // Run MI_MVE_FilterAndCSC()
	ret = MI_MVE_FilterAndCSC(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_FilterAndCSC() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
