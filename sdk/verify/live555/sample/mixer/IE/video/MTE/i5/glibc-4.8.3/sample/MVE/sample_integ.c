...
MI_RET Sample_Integ()
{
	...
	MVE_SRC_IMAGE_S src;
	MVE_DST_IMAGE_S dst;
    MVE_INTEG_CTRL_S ctrl =
    {
        .enOutCtrl = MVE_INTEG_OUT_CTRL_COMBINE
    };
    ...
    ret = MI_MVE_AllocateImage(handle, &dst, MVE_IMAGE_TYPE_U64C1, RAW_WIDTH, RAW_WIDTH, RAW_HEIGHT, "MVE_Output", MVE_MEM_ALLOC_TYPE_PHYSICAL);
    if (ret != MI_RET_SUCCESS)
    {
    ...
    // Run MI_MVE_Integ()
	ret = MI_MVE_Integ(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_Integ() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
