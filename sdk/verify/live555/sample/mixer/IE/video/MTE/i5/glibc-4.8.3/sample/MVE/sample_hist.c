...
MI_RET Sample_Hist()
{
	...
	if (handle == NULL)
    {
		...
		return MI_MVE_RET_INVALID_HANDLE;
    }
    ...
    ret = MI_MVE_AllocateBuffer(handle, &hist, sizeof(MVE_HIST_TABLE_MEM_S), "MVE_Output", MVE_MEM_ALLOC_TYPE_PHYSICAL);
    if (ret != MI_RET_SUCCESS)
    {
    ...
    // Run MI_MVE_Hist()
    ret = MI_MVE_Hist(handle, &src, &hist, 0);
    if(ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_Hist() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }

    // Save ouput data
    ret = Sample_SaveOutputBuffer(&hist, OUTPUT_NAME, sizeof(MVE_HIST_TABLE_MEM_S));
    if (ret != MI_RET_SUCCESS)
    {
