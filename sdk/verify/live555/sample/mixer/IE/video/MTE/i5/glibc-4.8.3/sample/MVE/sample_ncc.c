...
MI_RET Sample_NCC()
{
    ...
    // Run MI_MVE_NCC
    ret = MI_MVE_NCC(handle, &src0, &src1, &dst, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_NCC() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }

    // Save ouput data
    ret = Sample_SaveOutputBufferDirect((void*)&dst, OUTPUT_NAME, sizeof(dst));
    if (ret != MI_RET_SUCCESS)
    {
