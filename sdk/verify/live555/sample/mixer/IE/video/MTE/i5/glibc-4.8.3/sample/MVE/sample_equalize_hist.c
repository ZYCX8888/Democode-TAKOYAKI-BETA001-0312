/*
* sample_equalize_hist.c- Sigmastar
*
...
MI_RET Sample_EqualizeHist()
{
    ...
    // Allocate output buffer
    ret = MI_MVE_AllocateBuffer(handle, &ctrl.stMem, sizeof(MVE_EQUALIZE_HIST_CTRL_MEM_S), "MVE_Hist", MVE_MEM_ALLOC_TYPE_VIRTUAL);
    if (ret != MI_RET_SUCCESS)
    {
        printf("Can't allocate output buffer\n");
        goto RETURN_2;
    ...
    // Run MI_MVE_EqualizeHist()
	ret = MI_MVE_EqualizeHist(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_EqualizeHist() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
    ...
    // Save ouput data
    ret = Sample_SaveOutputBuffer(&ctrl.stMem, HIST_NAME, sizeof(MVE_EQUALIZE_HIST_CTRL_MEM_S));
    if (ret != MI_RET_SUCCESS)
    {
