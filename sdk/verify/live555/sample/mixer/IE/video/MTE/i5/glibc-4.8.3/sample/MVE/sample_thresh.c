...
MI_RET Sample_Thresh()
{
	...
	MVE_SRC_IMAGE_S src;
	MVE_DST_IMAGE_S dst;
    MVE_THRESH_CTRL_S ctrl =
    {
        .enMode    = MVE_THRESH_MODE_BINARY,
        .u8LowThr  = 128,
        .u8HighThr = 0,
        .u8MinVal  = 0,
        .u8MidVal  = 0,
    ...
    // Run MI_MVE_Thresh()
	ret = MI_MVE_Thresh(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_RET_SUCCESS)
    {
        printf("MI_MVE_Thresh() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
