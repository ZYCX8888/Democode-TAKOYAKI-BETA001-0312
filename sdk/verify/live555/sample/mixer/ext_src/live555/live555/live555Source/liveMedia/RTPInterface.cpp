...
RTPInterface::RTPInterface(Medium* owner, Groupsock* gs)
  ...
  int size = increaseSendBufferTo(envir(), fGS->socketNum(), 128*1024);
...
void RTPInterface::addStreamSocket(int sockNum,
  ...
  unsigned size = increaseSendBufferTo(envir(), sockNum, 768*1024);
  fprintf(stderr, "addStreamSocket socket: %d, size:%d\n", sockNum, size);	
	fflush(stderr);
}
