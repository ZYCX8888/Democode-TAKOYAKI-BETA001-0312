...
_UVC_Fill_Streaming_Control(struct uvc_streaming_control *ctrl,MI_S32 iframe, MI_S32 iformat);

static MI_S32 _UVC_FillBuffer(struct buffer *mem,UVC_IO_MODE_e io_mode)
{
    MI_UVC_BufInfo_t BufInfo;
...
retry:
    ...
    memset(&BufInfo,0x00,sizeof(MI_UVC_BufInfo_t));
...
static MI_S8 _UVC_Events_Process_Control(MI_UVC_Device_t *uvc,uint8_t req,uint8_t cs,
...
static MI_S8 _UVC_Events_Process_Streaming(MI_UVC_Device_t *uvc,uint8_t req, uint8_t cs,
                                            struct uvc_request_data *resp)
{
...
MI_S8 usb_vc_out_data(uint8_t entity, uint8_t cs, uint32_t len, struct uvc_request_data * data)
{
    switch (entity)
    {
    case UVC_VC_EXTENSION1_UNIT_ID:
 ...
 * */
static struct uvc_streaming_control
_UVC_Fill_Streaming_Control(struct uvc_streaming_control *c,MI_S32 iframe, MI_S32 iformat)
{
    MI_U32 nframes = 0;
