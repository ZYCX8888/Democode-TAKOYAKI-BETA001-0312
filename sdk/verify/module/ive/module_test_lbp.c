...
#define OUTPUT_NAME  "Output_LBP.raw"

MI_S32 ModuleTest_LBP()
{
    MI_S32 ret;
    ...
    MI_IVE_LbpCtrrl_t ctrl =
    {
        .eMode     = E_MI_IVE_LBP_CMP_MODE_NORMAL,
        .un8BitThr = 1
    };
    ...
    // Run MI_IVE_Lbp()
	ret = MI_IVE_Lbp(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_Lbp() return ERROR 0x%X\n", ret);
        goto RETURN_1;
    }
