...
MI_S32 ModuleTest_OrdStatFilter()
{
    MI_S32 ret;
    ...
    MI_IVE_OrdStatFilter_t ctrl =
    {
        .eMode = E_MI_IVE_ORD_STAT_FILTER_MODE_MEDIAN
    };
    ...
    // Run MI_IVE_OrdStatFilter()
	ret = MI_IVE_OrdStatFilter(handle, &src, &dst, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
