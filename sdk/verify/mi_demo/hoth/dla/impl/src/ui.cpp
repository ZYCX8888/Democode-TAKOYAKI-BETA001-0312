...
#define ASCII_WID 32
#define ASCII_HEI 32
void Ui::DataReceiver(void *pData, unsigned int dataSize, void *pUsrData)
{
    ...
    for (it = pDlaUiOsdInfo->begin(); it != pDlaUiOsdInfo->end(); ++it)
    {
        ...
        if (s32Ret == MI_RGN_OK)
        {
            ...
            for (unsigned int i = 0; i < intResCnt; i++)
            {
                ...
                u32Color = (nameLen)?(I4_GREEN):(I4_RED);
                if (stRectArea.u32X + stRectArea.u16PicW < it->uintWid
                    && stRectArea.u32Y + stRectArea.u16PicH < it->uintHei)
                {
                        ...
                        && (it->uintWid > nameLen * ASCII_WID + stRectArea.u32X))
                    {
                        ...
                        //printf("Draw %s\n", pRes[i].faceName);
                        ST_OSD_DrawText((MI_RGN_HANDLE)it->uintHandle, stTextPoint, pRes[i].faceName, I4_GREEN, DMF_Font_Size_32x32);
                    }
                }
            }
