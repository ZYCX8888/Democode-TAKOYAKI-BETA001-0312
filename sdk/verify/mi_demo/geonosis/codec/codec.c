    ...
    if ((E_AI_SOUND_MODE_MONO == eAiSoundMode) || (E_AI_SOUND_MODE_STEREO == eAiSoundMode))
    {
        while(FALSE == bAiExit)
        {
            ...
            if (MI_SUCCESS == s32Ret)
            {
				...
				else if ((E_AI_SOUND_MODE_STEREO == eAiSoundMode) && (bAiEnableSsl) && (!bAiEnableBf))
                {
...
int main (int argc, char *argv[])
{
    ...
    while ((s32Opt = getopt(argc, argv, "t:I::o:h::g::e::n::r:a:A::c:b::B::v:d:s:S::F::m:M:O::i:H::G::E::N::R:V:D:w:W::")) != -1)
    {
        switch(s32Opt)
        {
            // run time
