...
std::queue<ST_DATAINFO_T> g_queueData;

static MI_BOOL g_bRun;
static MI_BOOL g_bSetPop = FALSE;
...
static int gstOpened = 0;
static bool gbRequestIdr = false;
...
static void *_ST_VencGetData(ST_TEM_BUFFER stTemBuf)
{
	...
	ST_DATAINFO_T queData;
	bool blGet = false;
	if(0 == pstChnPort->u32ChnId)
	{	
		blGet = false;
		pthread_mutex_lock(&gstDataMuxCond.mutex);
		if(!g_queueData.empty())
		{
			memset(&stDataPackage, 0, sizeof(ST_DataPackage));
			queData = g_queueData.front();
			stDataPackage.u32DataLen = queData.len;
			if(gstOpened > 10)
			{
				g_queueData.pop();
				stDataPackage.pDataAddr = stDataPackage.pDataAddrStart = stDataPackage.pDataAddrAlign = queData.pData;
				if(g_queueData.empty())
				{
					gbRequestIdr = true;
				}
			...
			else
			{
				...
				memcpy(stDataPackage.pDataAddr, queData.pData, stDataPackage.u32DataLen);
				gstOpened++;
			}
		...
		if(gbRequestIdr)
		{
		 	MI_S32 iret =  MI_VENC_RequestIdr(pstChnPort->u32ChnId, TRUE);
			gbRequestIdr = false;
...
MI_S32 ST_StartPipeLine(MI_U8 i, MI_U32 u32Width, MI_U32 u32Height, MI_U32 u32CropW, MI_U32 u32CropH, MI_U32 u32CropX, MI_U32 u32CropY)
{
    ...
    if(pstStreamAttr[i].eType == E_MI_VENC_MODTYPE_H264E)
    {
        ...
        stChnAttr.stVeAttr.stAttrH264e.u32MaxPicWidth = VENC_MAX_WIDTH;
        stChnAttr.stVeAttr.stAttrH264e.u32MaxPicHeight = VENC_MAX_HEIGHT;
    ...
    else if(pstStreamAttr[i].eType == E_MI_VENC_MODTYPE_H265E)
    {
        ...
        stChnAttr.stVeAttr.stAttrH265e.u32MaxPicWidth = VENC_MAX_WIDTH;
        stChnAttr.stVeAttr.stAttrH265e.u32MaxPicHeight = VENC_MAX_HEIGHT;
    ...
    else if(pstStreamAttr[i].eType == E_MI_VENC_MODTYPE_JPEGE)
    {
        ...
        stChnAttr.stVeAttr.stAttrJpeg.u32MaxPicWidth =  VENC_MAX_WIDTH;
        stChnAttr.stVeAttr.stAttrJpeg.u32MaxPicHeight = VENC_MAX_HEIGHT;
...
void *Save_fastBootStream_func(void* p)
{
	MI_S32 s32Ret = MI_SUCCESS;
    MI_VENC_Stream_t stStream;
    MI_VENC_Pack_t stPack[16];
	...
	while(g_bRun)
	{
        ...
        if(MI_SUCCESS == s32Ret)
        {
			if(iCount >= 150)
			{
			    MI_VENC_ReleaseStream(0, &stStream);
				break;
			}
			else
			{
				stData.pData = (char*)malloc(stStream.pstPack[0].u32Len);
				stData.len = stStream.pstPack[0].u32Len;
				memcpy(stData.pData, stStream.pstPack[0].pu8Addr, stStream.pstPack[0].u32Len);
				pthread_mutex_lock(&gstDataMuxCond.mutex);
				g_queueData.push(stData);
				pthread_mutex_unlock(&gstDataMuxCond.mutex);
				iCount++;
			}
            MI_VENC_ReleaseStream(0, &stStream);
...
MI_S32 ST_VencStart(void)
{
    ...
    *************************************************/
    ...
    pthread_create(&g_ptSaveStream, NULL, Save_fastBootStream_func, NULL);
    return MI_SUCCESS;
}
...
void ST_DoCaptureBayerRAW(void *args)
{
	...
	int  TestLen = timage_resolution.u32image_width*timage_resolution.u32image_height*2;
	char * TestBuf = (char*)malloc(TestLen);
	g_expMode=0;
	MI_IQSERVER_GetSensorRawData(&TestBuf,&TestLen);
	//MI_IQSERVER_GetSensorRawData_ByWDMA6(0,&TestBuf,&TestLen);
